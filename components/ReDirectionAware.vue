...
const containerClass = computed(() => 
  cn('group/card relative h-60 w-60 overflow-hidden rounded-lg bg-transparent md:h-96 md:w-96', props.className)
);

const imageClass = computed(() => 
  cn('h-full w-full scale-150 object-cover transition-transform duration-300', props.imageClassName)
);

const childrenClass = computed(() => 
  cn('absolute bottom-4 left-4 z-40 text-white transition-opacity duration-300', props.childrenClassName)
);

const overlayClass = computed(() => 
  `absolute inset-0 z-10 bg-black/40 transition-opacity duration-300 ${
    direction.value === 'top' ? '-translate-y-full' : 
    direction.value === 'bottom' ? 'translate-y-full' : 
    direction.value === 'left' ? '-translate-x-full' : 
    direction.value === 'right' ? 'translate-x-full' : ''
  }`
);
